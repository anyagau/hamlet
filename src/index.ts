...
export const handleRequest: HandleRequest = async function (
  ...
  var queryData = parse(request.uri).query;
  var myColor = "maroon";
  if (queryData.color != undefined) {
    myColor != queryData.color;
  }
...
function parse(uri: string) {
  ...
  if (splits.length == 1) {
    return { path: splits[0], query: {} };
  } else {
    let query = splits[1];
    let queryObj = query.split("&").reduce(
      (acc, cur) => {
    ...
    return { path: splits[0], query: queryObj };
  }
}
